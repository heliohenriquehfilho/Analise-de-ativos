...
import webbrowser
import plotly.graph_objects as go
from PyQt5.QtWidgets import QListWidget, QFileDialog, QComboBox, QPushButton, QVBoxLayout, QHBoxLayout, QWidget, QTreeWidget, QTreeWidgetItem, QHeaderView, QProgressDialog
...
df_ativos["Preco"] = preco
df_ativos['Quant Min'] = []
df_ativos['Progresso'] = []
df_ativos['Data Compra'] = data_compra
df_ativos["Historico"] = Hist
df_ativos["Resultado"] = []
df_ativos['Total'] = []
df_ativos['Frequencia'] = frequencias
df_ativos['Div Total'] = []
df_ativos['Rentabilidade(%)'] = []
df_ativos['Data Compra'] = []
df_ativos["Preco Compra"] = Var
df_ativos["ROI"] = []

global arquivoPickle, pickles, name, dados
name = ""
...
class App(QMainWindow):
    ...
    def salvar_progresso(self):
        global Users, pickles, arquivoPickle, dados, df_ativos, username

        df = df_ativos.to_dict(orient="records")
        if self.nomeUsuario.text() == "":
            nome = username
            Users.append(nome)
        else:
            nome = self.nomeUsuario.text()
            Users.append(self.nomeUsuario.text())
        pickles[nome] = df
        with open(arquivoPickle, "wb") as arquivo:
            pickle.dump(pickles, arquivo)
            self.show_dialog("Progresso salvo com sucesso!")

    def carregar_usuario(self, username):
        global Users, pickles, arquivoPickle, dados
        ...
        if username in pickles:
            ...
            self.show_dialog("Usuário não encontrado")
